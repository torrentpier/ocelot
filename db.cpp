...
void mysql::load_users(user_list &users) {
	...
	try {
		...
		for (size_t i = 0; i < num_rows; i++) {
			...
			bool can_leech = true;
			user_ptr tmp_user = std::make_shared<user>(res[i][0], can_leech, protect_ip);
			auto it = users.insert(std::pair<std::string, user_ptr>(passkey, tmp_user));
			if (!it.second) {
				user_ptr &u = (it.first)->second;
				u->set_leechstatus(can_leech);
...
void mysql::record_peer(const std::string &record, const std::string &ip, const std::string &peer_id, const std::string &useragent, const std::string &hash) {
...
void mysql::flush_users() {
	...
	"INSERT INTO bb_bt_users (user_id, up_today, down_today, up_bonus_today, up_release_today) VALUES " + update_user_buffer +
...
void mysql::flush_peers() {
	...
	if (update_light_peer_buffer != "") {
		...
		sql = "INSERT INTO bb_bt_tracker (topic_id,tor_type,user_id,port,seeder,peer_hash,peer_id,ip,update_time) VALUES " +
					update_light_peer_buffer +
